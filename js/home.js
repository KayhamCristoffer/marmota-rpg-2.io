// ============================================================
// HOME.JS - Dashboard principal com Supabase
// ============================================================
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Expõe helpers para session-manager
...
function setupSidebar() {
  ...
  if (isAdmin(currentUser, currentProfile)) {
    ...
    document.getElementById('sidebarRole').classList.add('admin-role');
  }
...
async function loadDashboard() {
  try {
    ...
    const xpPct  = document.getElementById('xpPercent');
    const xpLbl  = document.getElementById('xpLevelLabel');
    const xpHint = document.getElementById('xpHint');
    if (xpFill) xpFill.style.width = `${pct}%`;
    if (xpPct)  xpPct.textContent  = `${Math.round(pct)}%`;
    if (xpLbl)  xpLbl.textContent  = `Nível ${lv}`;
    if (xpHint) xpHint.textContent = `${xp.toLocaleString('pt-BR')} / ${maxX.toLocaleString('pt-BR')} XP`;
    ...
    setValue('statQuests', completedCount);
...
function renderBadges(badges) {
  ...
  if (!grid) return;
  if (!badges.length) { grid.innerHTML = '<p class="no-badges">Nenhuma conquista ainda. Complete quests para desbloquear!</p>'; return; }
  grid.innerHTML = badges.map(b => {
    const ach = b.achievements;
    return `<div class="badge-item-full" title="${ach?.description || ''}">
      <span class="badge-icon">${ach?.icon_url || '🏅'}</span>
      <div><div class="badge-label">${ach?.title || 'Conquista'}</div><div class="badge-req">${ach?.description || ''}</div></div>
    </div>`;
  }).join('');
}
...
function renderQuests() {
  ...
  if (!quests.length) { grid.innerHTML = '<div class="empty-state"><i class="fas fa-scroll"></i><h3>Nenhuma quest disponível</h3></div>'; return; }
  ...
  grid.innerHTML = quests.map(q => {
    ...
    return `<div class="quest-card" data-type="${q.type}">
      ...
      <div class="quest-meta">
        ...
        ${q.min_level > 1 ? `<span class="quest-slots"><i class="fas fa-lock"></i> Nv ${q.min_level}+</span>` : ''}
      </div>
...
async function loadMyQuests() {
  ...
  try {
    const subs = await getUserSubmissions(currentUser.id);
    if (!subs.length) { list.innerHTML = '<div class="empty-state"><i class="fas fa-tasks"></i><h3>Nenhuma quest enviada ainda</h3><p>Vá para Quests e envie seu comprovante!</p></div>'; return; }
    const statusLabels = { pending:'Em Análise', approved:'Aprovada', rejected:'Rejeitada' };
    const typeIcons = { daily:'fa-sun',orange:'', weekly:'fa-calendar-week', monthly:'fa-calendar-alt', event:'fa-star' };
    list.innerHTML = subs.map(s => {
      const q = s.quests;
      return `<div class="my-quest-item">
        <div class="my-quest-icon" style="background:rgba(240,192,64,.1)">${q?.icon_url || '📜'}</div>
...
async function loadMaps() {
  ...
  try {
    ...
    if (!maps.length) { grid.innerHTML = '<div class="empty-state"><i class="fas fa-map"></i><h3>Nenhum mapa disponível</h3></div>'; return; }
    grid.innerHTML = maps.map(m => `
      <div class="map-card" data-type="${m.type || 'all'}">
        <div class="map-image">${m.icon_url || '🗺️'}</div>
        <div class="map-body">
          ...
          <div class="map-rewards">
            ...
            ${m.reward_tokens ? `<span class="map-reward-badge reward-tokens"><i class="fas fa-gem"></i> ${m.reward_tokens}</span>` : ''}
          </div>
...
async function loadRanking() {
  try {
    const data = await getRanking(currentPeriod);
    renderRanking(data);
    // Realtime
    if (rankingChannel) unsubscribeRanking(rankingChannel);
    rankingChannel = subscribeRanking(currentPeriod, renderRanking);
  } catch (err) {
    document.getElementById('rankingList').innerHTML = `<div class="empty-state"><i class="fas fa-exclamation-circle"></i><h3>Erro ao carregar ranking</h3></div>`;
  }
}
...
function renderRanking(data) {
  ...
  if (!data.length) { list.innerHTML = '<div class="empty-state"><i class="fas fa-trophy"></i><h3>Nenhum dado de ranking</h3></div>'; return; }
  ...
  podium.innerHTML = orders.filter(i => top3[i]).map(i => {
    const u = top3[i];
    const pos = i + 1;
    const avatar = u.icon_url ? `<img src="${u.icon_url}" class="podium-avatar" alt="${u.nickname}" onerror="this.style.display='none'">` : `<div class="podium-avatar">${(u.nickname||'?')[0].toUpperCase()}</div>`;
    return `<div class="podium-item pos-${pos}">
      ${pos === 1 ? '<div style="font-size:1.4rem;margin-bottom:4px">👑</div>' : ''}
      ${avatar}
      <span class="podium-name">${u.nickname}</span>
      <span class="podium-coins"><i class="fas fa-coins" style="font-size:.75rem"></i> ${u.score.toLocaleString('pt-BR')}</span>
      <div class="podium-stand">${pos}</div>
    </div>`;
  }).join('');

  const rest = data.slice(3);
  list.innerHTML = rest.map((u, idx) => {
    const pos = idx + 4;
    const isMe = u.id === currentUser?.id;
    return `<div class="ranking-item ${isMe ? 'is-me' : ''}">
      <span class="rank-position">${pos}</span>
      <div class="ranking-avatar">${u.icon_url ? `<img src="${u.icon_url}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">` : (u.nickname||'?')[0].toUpperCase()}</div>
      <span class="ranking-name">${u.nickname}${isMe ? ' 👈' : ''}</span>
      <span class="ranking-level">Nv ${u.level}</span>
      <span class="ranking-coins"><i class="fas fa-coins" style="font-size:.8rem;margin-right:4px"></i>${u.score.toLocaleString('pt-BR')}</span>
    </div>`;
  }).join('');
}

function setupRankingFilters() {
  document.querySelectorAll('#page-ranking .filter-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('#page-ranking .filter-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
...
async function loadProfile() {
  ...
  currentProfile = p;
  setValue('profileNickname', p.nickname || '');
  setValue('profileEmail',    currentUser.email || '');
  setValue('profileLevel',    `Nv ${p.level || 1}`);
  setValue('profileRole',     p.role === 'admin' ? '⚔ Admin' : '🗡 Usuário');
  const av = document.getElementById('profileAvatar');
  if (av) {
    if (p.icon_url) { av.style.backgroundImage = `url(${p.icon_url})`; av.style.backgroundSize = 'cover'; av.textContent = ''; }
    else av.textContent = (p.nickname || 'A')[0].toUpperCase();
  }
  const en = document.getElementById('editNickname');
  const eu = document.getElementById('editAvatarUrl');
  if (en) en.value = p.nickname  || '';
...
function setupProfile() {
  document.getElementById('saveProfileBtn')?.addEventListener('click', async () => {
    ...
    if (!nickname || nickname.length < 2) { showToast('Nickname deve ter pelo menos 2 caracteres', 'warning'); return; }
    btn.disabled = true; btn.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> Salvando…';
    try {
      await updateUserProfile(currentUser.id, { nickname, icon_url: iconUrl || null });
      currentProfile.nickname = nickname;
      currentProfile.icon_url = iconUrl;
      renderUserInSidebar(currentProfile);
      await loadProfile();
